 ...
 * @returns `(b ** e) mod m`
 */
export function powmod(b: anyint, e: anyint, m: anyint): anyint {
    const [zero, one, two] = [_BI(0), _BI(1), _BI(2)];
    let [bb, be, bm] = [_BI(b), _BI(e), _BI(m)];
    let br = one;
    while (zero < be) {
        if (be % two === one) br = mulmod(br, bb, bm);
        bb = mulmod(bb, bb, bm);
        be = halve(be);
    }
 ...
 * @param a
 */
export function millerRabinTest(a: number) {
    return function (n: anyint) {
        const [zero, one, two] = [_BI(0), _BI(1), _BI(2)];
 ...
 * 
 */
export function isMersennePrime(n: anyint) {
    let bn = _BI(n);
    if ((bn & (bn + _BI(1))) !== _BI(0)) return undefined;
    const bits = bn.toString(2).length;
    if (!isPrime(bits)) return false;
    let bs = _BI(4);
...
export function isqrt(n: anyint) {
    if (typeof n === 'number') return Math.floor(Math.sqrt(n));
    const $ = n.constructor;
    if (n == $(0)) return $(0);
    if (n == $(1)) return $(1);
    let xk = n;
...
export function jacobiSymbol(m: anyint, i: number): number {
    const $ = m.constructor;
    var j = 1
    var n = $(i < 0 ? -i : i);
...
export function isLucasProbablePrime(x) {
    ...
    let d = (() => {
        ...
        for (let i = 2, s = 1; i < 256; i++, s *= -1) {  // 256 is arbitrary
            d = s * (2 * i + 1);
            if (jacobiSymbol(x, d) === -1) return d;
        };
 ...
 */
export function primarityTest(n: anyint, nmrt = 0) {
    if (typeof n === 'number') {
    ...
    if (bn % b2 === b0) return [bn === b2, true];
    if (bn % b3 === b0) return [bn === b3, true];
    if (bn % b5 === b0) return [bn === b5, true];
    if (bn % b7 === b0) return [bn === b7, true];
    if (typeof BigInt === 'function') { // Baillie-PSW
        if (millerRabinTests.get(2)(bn) === false) return [false, true];
        if (isLucasProbablePrime(bn) === false) return [false, true];
        if (!nmrt) return [ true,
    ...
    for (const [p, test] of millerRabinTests.entries()) {
        ...
        if (millerRabinTests.get(p)(bn) === false) return [false, true];
        if (bn < A014233.get(p)) return [true, true];
    }
 ...
 */
export function isPrime(n: anyint) {
    const [primarity, certainity] = primarityTest(n)
    if (certainity) return primarity;
    throw RangeError(
        `${n} is probably${primarity ? ' ' : ' not '}a prime`
        + ' but not for sure.'
    );
}
/**
 ...
 * @param {anyint} n integer to check
 */
export const isProbablyPrime =
    (n: anyint, nmrt = 0) => primarityTest(n, nmrt)[0];
/**
 * find the next prime.
 * 
...
export const nextPseudoPrime =
    (n: anyint, nmrt = 0) => nextPrime(n, true, nmrt);
/**
...
export const previousPseudoPrime =
    (n: anyint, nmrt = 0) => previousPrime(n, true, nmrt);
/**
 * generates `n` primes. 
...
export function* primes(n = Number.POSITIVE_INFINITY) {
    const $ = n.constructor;
    for (let p = $(2), i = 0; i < n; i++, p = nextPrime(p)) yield p;
}
/**
 * generates primes where `begin <= p <= end`
 */
export function* primesBetween(
    begin: anyint = 2,
    end: anyint = Number.POSITIVE_INFINITY,
    unsure = false,
    nmrt = 0
) {
    if (end < begin) return primesBetween(end, begin, unsure, nmrt);
...
export function* pseudoPrimesBetween(
    begin: anyint = 2,
    end: anyint = Number.POSITIVE_INFINITY,
    nmrt = 0
) {
